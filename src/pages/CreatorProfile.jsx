...
const CreatorProfile = () => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const p = await getCreatorProfileBySlug(username);
      setProfile(p);
      if (p) {
  ...
  if (!profile) {
    return (
      <Layout mainClassName="max-w-xl mx-auto px-4 py-12 text-center">
        ...
        <Link to="/creator/apply" className="btn-primary inline-block mt-6 px-6 py-2.5 rounded-xl">Become a Creator</Link>
      </Layout>
    );
  }
  ...
  return (
    <Layout>
      <Helmet>
        <title>@{profile.slug} | {profile.displayName} | The Bharath News</title>
        <meta name="description" content={profile.bio || `${profile.displayName} on The Bharath News`} />
        <link rel="canonical" href={`https://thebharathnews.com/@${profile.slug}`} />
      </Helmet>
