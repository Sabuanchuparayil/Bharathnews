/**
 * Direct Supabase fill — bypasses Cloudflare Worker subrequest limits.
 * Usage: npm run fill:news:direct
 */
...
function extractImage(block) {
  ...
  if (enc && !/\.(mp3|mp4)(?:[?#]|$)/i.test(enc[1])) return enc[1];
...
const sources = [
  ...RSS_FEEDS.map(f => ({ ...f, type: f.url?.includes('news.google.com') ? 'googlenews' : 'rss' })),
  ...REGIONAL_RSS_SOURCES,
];
...
async function main() {
  ...
  for (const src of sources) {
    ...
    const category = src.category || 'india';
    ...
    for (const item of items.slice(0, 8)) {
      ...
      if (item.imageUrl || (imageUrl && !imageUrl.includes('unsplash.com'))) withRealImage++;
    ...
    process.stdout.write(`  ${src.name}: ${items.length} items\n`);
  }

  console.log(`\nDone: ${published} published, ${failed} failed, ~${withRealImage} with RSS/OG images`);
}
